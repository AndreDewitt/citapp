<section class="menu">
    <nav>
        <h5>Menú</h5>
        <ul>
            <li><a class="material-icons" href="http://localhost/Citapp/?controllers=negocioController&action=mostrar">store</a></li>
            <li><a class="material-icons" href="http://localhost/Citapp/?controllers=horarioController&action=mostrar">schedule</a></li>
            <li><a class="material-icons" href="http://localhost/citapp/?controllers=citaController&action=mostrar">splitscreen</a></li>
            <li><a class="material-icons" href="http://localhost/Citapp/?controllers=usuarioController&action=cerrar">logout</a></li>
        </ul>
    </nav>
</section>